...
const ProjectFormModal: React.FC<ProjectFormModalProps> = ({
  ...
  const onSubmit = (data: Omit<Project, "id" | "teamMember" | "isDeleted" | "resources">) => {
    let promise;
    let messages;
    ...
    if (mode === "create") {
      ...
      promise = dispatch(updateProjectAction({ id: project.id, data })).unwrap();
      messages = {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-end z-50 bg-black/50">
      <div className="bg-white h-full w-full max-w-md shadow-xl transform transition-all duration-300 flex flex-col">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex-grow p-6 space-y-4 overflow-y-auto">
          <div>
            <label htmlFor="projectName" className="block text-sm font-medium text-slate-600 mb-1.5">
              Project Name <span className="text-red-500">*</span>
            </label>
            <Controller
              name="projectName"
              control={control}
              render={({ field }) => <Input {...field} id="projectName" type="text" />}
            />
            {errors.projectName && <p className="text-red-500 text-sm mt-1">{errors.projectName.message}</p>}
          </div>
          <div>
            <label htmlFor="billingType" className="block text-sm font-medium text-slate-600 mb-1.5">
            ...
            <Controller
              ...
              control={control}
              render={({ field }) => (
                <select {...field} id="billingType" className="w-full p-2 border border-slate-300 rounded-md bg-slate-50 text-slate-800 focus:ring-2 focus:ring-[#714CDD] focus:border-[#714CDD]">
                  <option value="FixedCost">Fixed Cost</option>
                  <option value="TimeAndMaterial">Time and Material (TNM)</option>
                </select>
              )}
            />
            {errors.billingType && <p className="text-red-500 text-sm mt-1">{errors.billingType.message}</p>}
          </div>
          <div>
            <label htmlFor="creationDate" className="block text-sm font-medium text-slate-600 mb-1.5">
              Creation Date <span className="text-red-500">*</span>
            </label>
            <Controller
              name="creationDate"
              control={control}
              render={({ field }) => <Input {...field} id="creationDate" type="date" />}
            />
            {errors.creationDate && <p className="text-red-500 text-sm mt-1">{errors.creationDate.message}</p>}
          </div>
          <div>
            ...
            <Controller
              ...
              control={control}
              render={({ field }) => (
                <select {...field} id="status" className="w-full p-2 border border-slate-300 rounded-md bg-slate-50 text-slate-800 focus:ring-2 focus:ring-[#714CDD] focus:border-[#714CDD]">
