...
const Attendance: React.FC = () => {
    ...
    if (employeeCode && selectedYear) {
      const fetchAttendance = async () => {
        ...
        try {
          ...
          setAttendanceData(response.data.months);
          // New: Add a success toast for data fetching.
          toast.success(`Loaded attendance for ${selectedYear}.`, {
  ...
  // Modified: handleLeaveSubmit now provides a success toast.
  const handleLeaveSubmit = (leaveType: string) => {
    console.log("New leave type submitted:", leaveType);
    toast.success(`Leave type "${leaveType}" submitted successfully!`, {
      className: "bg-blue-50 text-blue-800",
    });
    setIsLeaveModalOpen(false);
  };

  // Modified: handleRequestSubmit now provides a success toast.
  const handleRequestSubmit = (formData: any) => {
    console.log("New leave request data:", formData);
