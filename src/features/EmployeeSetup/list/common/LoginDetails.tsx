...
const LoginDetails: React.FC<LoginDetailsProps> = ({ employee }) => {
  ...
  // New: useEffect to show a toast when a general error occurs.
  useEffect(() => {
    if (error) {
  ...
  // Modified: handleAddSubmit is now async and uses toasts.
  ...
  // Modified: handleEditSubmit is now async and uses toasts.
  ...
  return (
    <div className="mt-6">
      ...
      {/* Modified: Removed the inline error message. */}
      <div className="space-y-2">
        {details ? (
          <>
            <ResponsiveDetailItem label="Username" value={details.username} />
