...
const EmployeesTable: React.FC = () => {
  ...
  // New: useEffect to show a toast on fetch error
  useEffect(() => {
    if (error) {
  ...
  // Modified: useEffect for invite status now uses toasts
  useEffect(() => {
    if (inviteStatus === "succeeded") {
  ...
  // Modified: Converted to async function to handle toast notifications
  const handleConfirmAction = async () => {
    if (!employeeForModal || !actionToConfirm) return;

    // A reusable toast configuration for async operations
    ...
    try {
      switch (actionToConfirm) {
        ...
        case "Invite":
        case "Re-invite":
          // The inviteStatus useEffect will handle success/failure toasts.
          // We just dismiss the loading toast here.
          await dispatch(sendInviteEmail(employeeForModal.code)).unwrap();
          toast.dismiss(toastId);
          break;
        default:
          toast.dismiss(toastId); // Dismiss if no action is matched
          break;
      }
    } catch (err: any) {
      // Handle errors from any of the async thunks
      toast.error(
  ...
  const renderTableContent = () => {
    if (loading) return <div className="text-center p-10">Loading...</div>;
    // The error toast is now handled by the useEffect
    if (error)
      return (
