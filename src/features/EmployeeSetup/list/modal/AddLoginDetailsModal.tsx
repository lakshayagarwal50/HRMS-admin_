...
// import type { CreateLoginDetailsPayload } from "../../../../store/slice/loginDetailsSlice"; // Adjust path
...
//   // --- STYLING CHANGE: Applied from the original AddLoanModal ---
...
const AddLoginDetailsModal: React.FC<AddLoginDetailsModalProps> = ({
  ...
  // Modified: handleSubmit now triggers a toast on validation failure.
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validate()) {
      onSubmit({ username, password, loginEnable, accLocked });
    } else {
      // New: Show a toast notification if validation fails.
