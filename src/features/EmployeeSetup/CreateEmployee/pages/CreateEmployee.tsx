...
//     (state: RootState) => state.departments.status
//   );

//   const designations = useAppSelector(
//     (state: RootState) => state.employeeDesignations.items
//   );
//   const designationStatus = useAppSelector(
//     (state: RootState) => state.employeeDesignations.status
//   );

//   const leaveSetups = useAppSelector(
//     (state: RootState) => state.leaveSetups.items
//   );
//   const leaveSetupsStatus = useAppSelector(
//     (state: RootState) => state.leaveSetups.status
//   );

//   const locations = useAppSelector((state: RootState) => state.locations.items);
//   const locationsStatus = useAppSelector(
//     (state: RootState) => state.locations.status
//   );

//   const salaryStructures = useAppSelector(
//     (state: RootState) => state.salaryStructures.data
//   );
//   const salaryStructuresStatus = useAppSelector(
//     (state: RootState) => state.salaryStructures.status
//   );

//   const workingPatterns = useAppSelector(
//     (state: RootState) => state.workingPatterns.items
//   );
//   const workingPatternsStatus = useAppSelector(
//     (state: RootState) => state.workingPatterns.status
//   );

//   const holidayConfigurations = useAppSelector(
//     (state: RootState) => state.holidayConfigurations.items
//   );
//   const holidayConfigurationsStatus = useAppSelector(
//     (state: RootState) => state.holidayConfigurations.status
//   );

//   const createStatus = useAppSelector(
//     (state: RootState) => state.createEmployee.createStatus
//   );
//   const createError = useAppSelector(
//     (state: RootState) => state.createEmployee.createError
//   );
...
//   const getPayslipComponentOptions = () =>
//     salaryStructures.map((salary) => salary.groupName);
//   const getWorkingPatternOptions = () =>
//     workingPatterns.map((pattern) => pattern.name);
//   const getHolidayGroupOptions = () =>
//     holidayConfigurations.map((config) => config.groupName);
...
//                   .filter((des) => des.status === "active") // (optional) sirf active
//                   .map((des) => des.designationName)} // <-- yahi bug tha
//                 value={formData.designation}
//                 onChange={handleChange}
//                 disabled={
//                   designationStatus === "loading" || !formData.department
//                 }
//               />
...
import { fetchRoles } from "../../../../store/slice/roleSlice"; // --- 1. IMPORT fetchRoles ---
import { type NewEmployee } from "../../../../types/index";
import { type RootState } from "../../../../store/store";

// Static mock dropdown options (role removed)
const staticDropdownOptions = {
  title: ["MR", "MRS", "MS"],
  gender: ["Male", "Female", "Other"],
  reportingManager: [
...
const CreateEmployeeForm: React.FC = () => {
  ...
  // --- 2. SELECT ROLES STATE ---
  const roles = useAppSelector((state: RootState) => state.roles.items);
  const rolesStatus = useAppSelector((state: RootState) => state.roles.status);
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
              ...
              <SelectField
                ...
                name="designation"
                options={designations
                  .filter((des) => des.status === "active")
                  .map((des) => des.designationName)}
                value={formData.designation}
                onChange={handleChange}
                disabled={
                  designationStatus === "loading" || !formData.department
