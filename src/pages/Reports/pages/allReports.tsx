...
const AllReports: React.FC = () => {
  ...
  // This state holds the live value from the input
  const [searchQuery, setSearchQuery] = useState(currentSearch);
  // This debounced value triggers the URL update
  const debouncedSearchQuery = useDebounce(searchQuery, 500);

  // This effect fetches data whenever the URL changes
  useEffect(() => {
    dispatch(
      fetchAllReports({
        page: currentPage,
  ...
  const handleLimitChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // Set new limit and reset to page 1, keeping the current search term
    setSearchParams({ limit: newLimit, page: "1", search: currentSearch });
  };

  const handlePageChange = (newPage: number) => {
    // Keep other params when changing page
    setSearchParams({
      page: String(newPage),
      limit: String(currentLimit),
      search: currentSearch,
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      // Refetch data with current params after delete
      dispatch(
        fetchAllReports({
          page: currentPage,
          limit: currentLimit,
