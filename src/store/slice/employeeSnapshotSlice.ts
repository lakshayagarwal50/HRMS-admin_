...
export const fetchEmployeeSnapshot = createAsyncThunk<
  FetchedData,
  // ADD search TO THE ARGUMENTS
  { page?: number; limit?: number; search?: string; filters?: Record<string, any> },
  { rejectValue: string }
>(
  'employeeSnapshot/fetchReport',
  async (params, { rejectWithValue }) => {
    // DESTRUCTURE search FROM PARAMS
    const { page = 1, limit = 10, search, filters = {} } = params;
    try {
      const queryParams: Record<string, any> = { page, limit, ...filters };

      // CONDITIONALLY ADD search TO THE API CALL
      if (search) {
